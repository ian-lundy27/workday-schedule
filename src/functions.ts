import type { Rule, ICSEvent, Calendar, Timestamp, MeetingPattern } from "./types"
import { Days, Rules, Timezone } from "./enums"
import type { RequiredInfo } from "./test"
...
function eventToString(event: ICSEvent): string {
    ...
    if (!hasUid) retString += Rules.UID + ":" + generateUid() + "\n"
    return retString + "END:VEVENT"
}

function calendarToString(cal: Calendar): string {
    if (cal.length === 0) throw new Error("Empty calendar object (no events read from file)")
...
function createRule(type: string, val: string): Rule {
    return {
        key: type,
        val: val
    }
 ...
 */
function buildRRuleValString(params: {[key: string]: string}): string {
    let arr: string[] = []
 ...
 */
function timestampToString(s: Timestamp): string {
...
function isoToTimestamp(stamp: string): Timestamp {
 ...
 */
function isMeetingDays(text: string): boolean {
 ...
 */
function meetingDaysToEnum(text: string): Days[] {
    let days: Days[] = []
    for (const char of text) {
        switch (char) {
            case "M":
 ...
 */
function amPmToTimestamp(text: string): Timestamp {
 ...
 */
function isValidDate(date: Timestamp): boolean {
 ...
 * @returns true if timestamp has hour, minute, and second props, false otherwise
 */
function isValidTime(time: Timestamp): boolean {
 ...
 */
function parseMeetingCellBlock(text: string): MeetingPattern {
    let pattern = {
        days: [] as Days[],
        start: {} as Timestamp,
        end: {} as Timestamp,
 ...
 */
function parseMeetingCell(text: string): MeetingPattern[] {
...
function patternToRRuleParams(pattern: MeetingPattern, end: string): {[key: string]: string} {
    return {
        FREQ: "WEEKLY",
        INTERVAL: "1",
...
function combineDateTime(date: Timestamp, time: Timestamp): Timestamp {
    return {
        year: date.year,
        month: date.month,
        day: date.day,
...
function icsDateTime(type: Rules, timestamp: string, endpoint: Timestamp, timezone: Timezone): string {
    return type + ";TZID=" + timezone + ":" + 
        timestampToString(combineDateTime(isoToTimestamp(timestamp), endpoint))
}

function reqInfoToEvent(info: RequiredInfo, timezone: Timezone): ICSEvent[] {
    let events: ICSEvent[] = []
...
export async function convertXlsxPathToIcsString(file: File, timezone: Timezone): Promise<string> {
